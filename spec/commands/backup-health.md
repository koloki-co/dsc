...
Each scan permits at most 1,000 service pages (up to one million objects for full pages); completion on the last allowed page succeeds. A still-truncated final page, repeated token, missing/invalid completion flag or continuation token, malformed object, or failed AWS invocation fails the entire scan. The forum receives `inaccessible` and the command exits non-zero, without publishing partial totals or a partial newest archive. Token history retains at most 1,000 SHA-256 fingerprints, not arbitrarily large token strings.

Each `aws` invocation uses piped stdout/stderr. Stdout is capped at 64 MiB (inclusive); excess output fails immediately rather than waiting for the child to exit. Stderr is continuously drained and discarded, not closed at a diagnostic cap. Both pipes are read in fair, fixed-size chunks using readiness checks (Unix `poll`, Windows `PeekNamedPipe`), without reader threads or blocking joins. Nonzero exit status and pipe-read errors are preserved; raw arguments and stderr are omitted from diagnostics because they may echo credentials or opaque continuation tokens.

The call budget is the smaller of 60 seconds and the bucket scan's remaining 15-minute budget. An exhausted budget starts no new call. The deadline covers pipe EOF as well as child exit, and a final page cannot publish a successful summary after expiry. Saturating duration subtraction avoids overflow/underflow. On failure, pipes are closed and the direct child is killed if necessary and reaped; cleanup failures are reported alongside the original error. Descendants retaining inherited pipes cannot block return, but whole-process-tree termination is not promised. OS process creation/termination and in-memory JSON parsing are not preemptible, so these are execution budgets rather than hard real-time guarantees. Shared buckets are currently scanned per forum with that forum's credentials, not deduplicated. Independent buckets are scanned through a bounded worker pool (width derived from the fleet worker policy, ceiling 4): rows whose status was already resolved during the configuration phase stream immediately, and rows still needing a scan stream fastest-first as each bucket's scan completes, so one slow bucket no longer holds up the others' output.
...
- [x] R52/P13: bound each `aws` subprocess call with a 64 MiB stdout cap and a 60-second per-call timeout (killing a hung call), plus a 15-minute overall deadline per bucket scan; real-subprocess fixture tests (`tests/fixtures/fake-aws`) cover the byte cap and the timeout kill.
- [x] PR #140 review: continuously drain both pipes, reject oversize output promptly, bound descendant-held pipes without reader threads, preserve failures without echoing secrets, fingerprint token history, and enforce the remaining scan budget including the final page. Offline tests cover noisy stdout/stderr, exact caps, invalid JSON/nonzero exits, early EOF, inherited pipes, direct-child reaping, and deterministic injected-clock deadline boundaries.
- [x] R52/P13: scan independent buckets through a bounded worker pool instead of serially, streaming already-resolved rows immediately and scanned rows fastest-first as each completes; a real-subprocess test proves concurrent jobs don't cross-wire results.
