...
Each scan permits at most 1,000 service pages (up to one million objects for full pages); completion on the last allowed page succeeds. A still-truncated final page, repeated token, missing/invalid completion flag or continuation token, malformed object, or failed AWS invocation fails the entire scan. The forum receives `inaccessible` and the command exits non-zero, without publishing partial totals or a partial newest archive. The page budget also bounds retained continuation-token history. Each `aws` invocation is spawned with piped stdout/stderr rather than `Command::output()`: stdout is capped at 64 MiB (a service page is at most 1,000 objects, so this bounds memory against a nonconforming provider rather than real AWS traffic) and the call is killed if it does not complete within 60 seconds, so a hung CLI or unresponsive endpoint cannot block a scan indefinitely. A bucket's whole scan (however many pages it takes) additionally carries a 15-minute wall-clock budget, checked once per page. Shared buckets are currently scanned per forum with that forum's credentials, not deduplicated, and independent buckets are still scanned serially rather than through a bounded concurrent pool.
