...
> **Status: Phases 1 and 2 implemented (unreleased); R65 fleet execution in progress.** `list`, `show`, and `run` use the canonical bundled-plugin routes with safe pagination, typed structured output, parameter files, exact query export, CSV download, explain, limits, filtering, and sorting. R65 adds exact query-name resolution and fleet execution as the first report-evidence primitive. Managed query mutation remains deliberately separate.
...
dsc explorer run <discourse>|--all|--tags <tags> --query-name <exact-name> [--params <json>|--params-file <file>] [--explain] [--limit <n>] [--format text|json|yaml]
```
...
- **Exact-name and fleet execution (R65)** - `--query-name` resolves one complete case-sensitive query name from each selected forum's fully paginated catalogue. Zero matches and duplicate exact matches are explicit per-forum failures. `--all` and `--tags` require `--query-name`, use the shared bounded fleet executor, preserve configuration order, retain success/error rows, and return non-zero after complete output on partial failure. Numeric IDs and CSV remain single-forum because neither is portable without additional destination semantics. See [report evidence collection](report-evidence.md).
