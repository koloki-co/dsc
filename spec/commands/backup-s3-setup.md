...
> `--use-iam-profile`, the policy and user/key creation are skipped and
...
- Unless `--no-test`, runs a `dsc backup create` and confirms the new object appears in the bucket, then reports pass/fail. Verification records the trigger time, polls bounded `list-objects-v2` pages (not a full recursive listing) for up to three minutes, printing per-attempt progress, and stops at the first backup archive with `LastModified` strictly after the trigger time, so pre-existing archives in a reused bucket are never mistaken for proof the new backup landed (P14, spec/performance-audit.md). If every attempt fails (bad region, wrong credentials), the poll reports the last listing error instead of a bare "nothing visible"; a transient failure is retried within the three-minute window.
