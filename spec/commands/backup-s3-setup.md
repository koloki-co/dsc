...
- Unless `--no-test`, runs a `dsc backup create` and confirms the object appears in the bucket, then reports pass/fail. Verification polls bounded `list-objects-v2` pages (not a full recursive listing) for up to three minutes, printing per-attempt progress and stopping at the first `.tar.gz` archive found (P14, spec/performance-audit.md).
