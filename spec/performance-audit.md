...
**PR #139 review fixes:** the scan now rejects a missing/non-boolean `IsTruncated` instead of silently publishing a partial summary, and caps each scan at 1,000 service pages so unique-token streams cannot run indefinitely or grow token history without bound. Offline tests exercise the actual scan loop and argument construction, cross-page aggregation/ties, empty intermediate pages, later-page failures, malformed pagination, repeated tokens, and completion exactly at the budget. Per-page subprocess output remained buffered without a byte cap or overall deadline; these were not solved by paging.

**Addressed 2026-09-13, including PR #140 review:** `run_aws_json` caps stdout at 64 MiB and continuously drains/discards stderr in fair, readiness-checked chunks. Unlike the initial reader-thread implementation, it rejects oversize output immediately and cannot hang joining readers when descendants inherit pipes. The per-call budget is the smaller of 60 seconds and the remaining 15-minute bucket budget; the final page is checked before publishing success. Cleanup closes pipes and kills/reaps the direct child, reporting cleanup errors without hiding the original failure. Diagnostics retain exit status and I/O error context without echoing raw arguments or stderr that may contain secrets. Token history stores bounded SHA-256 fingerprints. Real-subprocess fixtures cover noisy streams, exact caps, inherited pipes, early EOF, exit failures, secret suppression, and direct-child reaping; injected-clock tests deterministically cover cumulative and final-page deadlines. Whole-process-tree termination and preemption of OS calls/JSON parsing are not promised. See [backup-health](commands/backup-health.md) for the contract.

**Addressed 2026-09-14 (bounded pool):** independent bucket scans now run through `scan_s3_jobs`, a bounded worker pool (`fleet_worker_count`-derived width, ceiling 4) built on the same shared-queue/`thread::scope` pattern as `common::run_fleet`. Rows that need no S3 call (already resolved at the configuration phase) stream immediately; rows still `unknown` are scanned concurrently, with results applied and streamed on the calling thread as each bucket's scan completes (fastest-first), so one slow bucket no longer holds up the others. A real-subprocess test (`scan_s3_jobs_runs_independent_buckets_concurrently_without_cross_wiring`) drives six buckets through two workers via the `fake-aws` fixture - extended to echo the requested `--bucket` name back into the returned object key - and asserts each job's result carries its own bucket's data, not another job's. **P13 is now fully addressed.**
...
**Addressed 2026-09-15:** each poll now walks bounded `list-objects-v2` pages through `backup`'s `run_aws_json` (shared, not duplicated - now `pub(crate)` alongside its `MAX_S3_PAGE_BYTES`/`AWS_CALL_TIMEOUT`), replacing `aws s3 ls --recursive`'s unbounded `Command::output()`. Each page is its own byte-capped, explicitly-timed subprocess call rather than one arbitrarily long-running whole-bucket dump, and the walk stops at the first `.tar.gz` key instead of folding the entire bucket, so a bucket already holding earlier backups (the common `--reuse-user` case) resolves from its first, oldest page. A per-attempt line now prints elapsed time. A transient `aws` failure (throttling, IAM propagation delay) is tolerated within the outer three-minute deadline rather than failing `setup-s3` outright, preserving the previous non-fatal behavior. Real-subprocess tests via the `fake-aws` fixture cover an existing archive being found and an empty bucket reporting no match; the JSON page-matching logic is separately unit-tested against constructed pages.
...
**Addressed 2026-09-11, reviewed in PR #138 (dsc 0.19.0):** The four path-following loops (category topics, deleted-topic list, private-message list, group fallback list) share `MAX_PAGINATION_PAGES` (1,000) in `src/api/client.rs`. The initial page counts; a terminal page 1,000 succeeds, while a continuation requiring page 1,001 fails before that request. Existing cycle detection now compares JSON-normalized paths, including the initial category path. Category/deleted-topic/PM pages with explicit continuations are followed even when empty. Group fallback intentionally stops on an empty page: upstream [GroupsController#index](https://github.com/discourse/discourse/blob/main/app/controllers/groups_controller.rb) (source checked 2026-09-11, not a live-install observation) emits `load_more_groups` unconditionally, including after exhaustion. Data Explorer and webhook listing retain their independent caps.

`user activity` has a 100,000-item safety budget whenever `--limit` is absent, **including with `--since`**: a date cutoff does not bound item count. Exactly 100,000 matching items succeed if the following page is empty or past the cutoff; the first additional matching item fails before being retained, even on a page that crosses the cutoff. An explicit `--limit` replaces this budget, may exceed 100,000, and a zero limit emits an empty result without fetching. Offset advancement uses the actual response length, not an assumed minimum page size, with checked overflow. This also fixes pre-existing silent skipping in `dsc user activity <forum> <user>` when the server returns fewer than 30 rows per page. No new `--all` flag or configurable page budget is introduced: these fixed safety limits are the deliberately minimal implementation, not a new full-export interface.

Budget, cycle, HTTP, and parse failures propagate without rendering partial list results; category pull fails before creating its output directory on pagination errors. Per-request timeouts, capped response bodies, bounded 429 retries, and normal process-level SIGINT termination remain unchanged. These are page/item limits, not a tight aggregate byte or wall-clock deadline: large page bodies, per-topic detail fetches, and retry waits can still make a bounded traversal expensive. Streaming output remains P22. `tests/pagination-budget-test.rs` covers exact page/item boundaries, cumulative pagination, explicit limits, cutoff handling, short pages, cycles, empty continuations, and later response failures; CLI subprocesses have test deadlines so a regressed endless loop fails rather than hanging the suite.
