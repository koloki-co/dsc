...
**Addressed 2026-09-13:** `aws_json` (now `run_aws_json`, parameterised for tests) spawns the `aws` subprocess with piped stdout/stderr instead of `Command::output()`, so it can bound both: stdout is capped at 64 MiB (a service page is at most 1,000 objects, so this bounds memory against a nonconforming provider rather than real AWS traffic) and the call is killed if it does not complete within 60 seconds via a `try_wait` poll loop, replacing the previous unbounded blocking `wait()`. `scan_s3_bucket` additionally carries a 15-minute wall-clock deadline for the whole bucket scan (checked once per page), so a bucket needing many slow-but-not-hung pages cannot run away even though no single call hangs. Real-subprocess fixture tests (`tests/fixtures/fake-aws`, installed via a `FakePathGuard` shared with `commands::ssh`'s existing `FakeSshPath` PATH-lock mechanism so the two fixtures cannot race on `PATH`) cover the byte cap and the timeout kill against an actually spawned process; the deadline is covered by an offline `scan_s3_bucket` test with an injected slow closure. **Outstanding:** the bounded-pool half of the original recommendation (scanning independent buckets concurrently) remains open (see roadmap R52). See [backup-health](commands/backup-health.md) for the contract.
