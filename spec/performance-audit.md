...
**Addressed 2026-09-11:** The four path-following loops (category topics, deleted-topic list, private-message list, group fallback list) now share one `MAX_PAGINATION_PAGES` (1,000) constant in `src/api/client.rs`, alongside the existing loop-detection `HashSet`, each erroring with a named-limit message rather than following an endless run of unique continuation paths - matching the cap already used independently by the Data Explorer (`explorer.rs`) and webhook (`webhooks.rs`) pagination loops, which this does not touch. `user_activity`'s offset-based history loop (`src/commands/user.rs`) gained a matching `MAX_UNBOUNDED_ACTIVITY_ITEMS` (100,000) item cap that applies only when neither `--limit` nor `--since` is given - an explicit `--limit` remains the caller's own bound and is never overridden. Omitting both flags was already the CLI's documented "fetch everything" signal, so no new `--all` flag was added; a clear error naming the cap and suggesting `--limit`/`--since` replaces the prior unbounded fetch, consistent with the recommendation against a silent truncation. Streaming output (rather than full retention) is not addressed here - see P22. Five regression tests in `tests/pagination-budget-test.rs` drive each site against a mock that always returns a fresh, unique continuation and assert a clear "exceeded" error.
