...
- [~] **R50 - Pre-announcement security hardening** - from the 2026-08-25 pre-announcement review. Implemented on `main`: the API client now follows redirects only within the configured origin (reqwest strips only `Authorization`/`Cookie`/`Proxy-Authorization`/`WWW-Authenticate` cross-host, so the custom `Api-Key` header would otherwise have been replayed to any host a forum redirected to - an admin-key disclosure path); `dsc config check` warns when `baseurl` is plaintext `http://` (loopback exempt); `render_shell_template` substitutes single-pass so an inserted value can no longer be reopened by a later placeholder pass; the inline emoji preview client has a timeout; `.gitignore` covers SAR bundles and backup archives. Remaining: (1) cap response body reads - every JSON response is currently read via unbounded `.text()`, so a hostile or malfunctioning host can drive memory use until the 120s request timeout; the streaming backup download path is correctly exempt. (2) `dsc` restricts file modes to `0600`/`0700` under `#[cfg(unix)]` only, so a Windows `dsc.toml` inherits default ACLs despite the shipped MSI - either restrict the ACL or document the gap in [configuration](../docs/configuration.md).
...
- [~] **R13 - `backup setup-s3` Phase 2/3** - Phase 2 is implemented on `main` but not yet released: `--use-iam-profile` (EC2 instance role, no static keys), `--all`/`--tags` fleet fan-out, and `--reuse-user` key rotation (mints a fresh access key for the existing derived user, re-points Discourse at it, and deactivates the key(s) that existed before rotation). Remaining: Phase 3's native AWS SDK backend and `--retention` lifecycle. Spec: [backup-s3-setup](commands/backup-s3-setup.md).
- [x] **R41 - `dsc backup health`** - fleet S3 evidence for newest backup timestamp/age, newest archive size, total bucket bytes, and object count. Text/CSV rows stream as provider queries complete; JSON/YAML remain buffered. Staleness follows each forum's `backup_frequency`; S3-compatible endpoints and process-local credentials are read from site settings, verified read-only against WeAllCount on DigitalOcean Spaces. Later monitoring and remediation phases remain demand-driven. Spec: [backup-health](commands/backup-health.md).
