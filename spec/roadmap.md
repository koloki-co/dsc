...
- [ ] **R49 - `dsc topic change-owner`** - reassign the visible author of a topic's posts (Discourse admin `PUT /t/{id}/change-owner` endpoint). No `dsc` command currently covers this; the workaround today is the Admin UI wrench icon → Change owner. Driver: content compiled from a staff member's source document, posted via `dsc topic new` under the operator's own admin account, then needing reassignment to the actual author (ACCM Discourse, `kitchen.culinarymedicine.org`). Spec: [topic-post-ownership](commands/topic-post-ownership.md).
