...
The single list of planned and in-progress work for `dsc`. Shipped history lives in [CHANGELOG.md](../CHANGELOG.md).
...
- [~] **R11 - `category` definition sync Phase 2/3** - Phase 1 shipped the blocking round-trip (`category def pull/push`, `category show/get/set`) for category definitions: description, permissions, position, topic template, and tag rules. It normalises description terminal newlines, rejects unknown YAML keys, and names changed fields in dry-run plans. `category rename` (safe id-based rename) has shipped; explicit live `category diff` is implemented; `category set` now supports `--append`/`--remove` for list fields (`allowed_tags`, `allowed_tag_groups`). Phase 2 now round-trips `required_tag_groups`, category type IDs, and complete category custom-field maps. `parent` now resolves by name as well as slug, and `def push`/`dry-run` validate every entry's parent reference against the server up front - one clear error naming every unresolvable parent, instead of a mid-push 4xx after earlier entries were already written. Remaining work: display/assets and prune. Driver: York Music Marketplace needs the Discourse `support` category type to enable accepted answers, not an ignored `solved_enabled` category field. Spec: [category-definition-sync](commands/category-definition-sync.md).
...
- [~] **R13 - `backup setup-s3` Phase 2/3** - `--use-iam-profile` (EC2 instance role, no static keys) and `--all`/`--tags` (fleet-wide fan-out, mirroring `backup create --all`) have shipped. Remaining: `--reuse-user` (key rotation); then a native AWS SDK backend and `--retention` lifecycle. Spec: [backup-s3-setup](commands/backup-s3-setup.md).
...
- [x] **R46 - Palette push dry-run plan** - `dsc theme palette push --dry-run` now validates the source, resolves an existing palette where relevant, prints the exact create/update payload and request sequence, and avoids all server and local writes. Driver: copying York Music's approved Marigold and Marigold Dark palettes safely before applying the live changes. Spec: [theme-management](commands/theme-management.md).
