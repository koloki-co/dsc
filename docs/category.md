...
- `style_type` is `square` (colour swatch, the default), `icon`, or `emoji`. `icon` is a FontAwesome icon name (used when `style_type` is `icon`); `emoji` is an emoji shortcode without colons (used when `style_type` is `emoji`) - e.g. `dsc category set forum1 support style_type icon`, `dsc category set forum1 support icon life-ring`. Logo/background image assets are out of scope (asset upload is a separate problem).
