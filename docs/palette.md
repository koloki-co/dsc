...
`--dry-run` prints the complete create or update plan: the resolved palette, every colour in a create payload or changed field in an update, the API requests, and the assigned-ID and refreshed local snapshots a create would write. It performs the read required to check an existing palette but sends no `POST` or `PUT` request and does not modify the local file.
